...
public class BokManagerCalendarDaoImpl implements BokManagerCalendarDao {
    ...
    public void initTable() {
        ...
        logger.info("--- {}\n", sql.toString());
        logger.info("--- 기념일 DB 초기화(이미 존재하는 경우 무시) result=[{}]", jdbcTemplate.update(sql.toString())); 
    }
    ...
    public int insertItem(BokManagerCalendarHolidayDto dto) {
        ...
        int result = jdbcTemplate.update(sql.toString(), dto.getCalYear(), dto.getCalMonth(), dto.getCalDay(), dto.getCalName(), dto.getCalData());
        logger.info("--- 기념일 DB 단건추가 RESULT : [{}]", result);
        return result;
    }
    ...
    public List<BokManagerCalendarHolidayDto> selectItems(int year, int month, String name) {
        ...
        logger.info("--- {}\n", sql.toString());
        logger.info("--- 기념일 DB 리스트 조회 RESULT CNT : " + retValue.size());
        return retValue;
    }
